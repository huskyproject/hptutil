...
int SquishFixArea()
{
    ...
    harea = MsgOpenArea((UCHAR *) NewNameBase, MSGAREA_CREATE, 
			MSGTYPE_SQUISH | MSGTYPE_NOTH);
	...
	if (findHdrId(SqdHandle) != -1) {
	    if (read_sqhdr(SqdHandle, &sqhdr) == 0) break;
	    ...
	    if (hmsg) {
		MsgWriteMsg(hmsg, 0, &xmsg, (byte *) text,
			   (dword) strlen(text),
			   (dword) strlen(text),
			   (dword)strlen(ctlbuf), (byte *)ctlbuf);
		fixmsg++;
	    }
...
int InitSmapi(s_fidoconfig *config)
{
    ...
    if (config == NULL) defzone = 2;
    else defzone = (UINT16) config->addr[0].zone;
    m.req_version = 0;
    m.def_zone = defzone;
    if (MsgOpenApi(&m) == 0) return 0;
    return 1;
}
...
int fixArea(s_fidoconfig *config)
{
    ...
    if (InitSmapi(config) == 0) {
        if (typebase == 1) rc = SquishFixArea();
