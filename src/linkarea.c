        ...
        for(i = 0; i < msgs; i++)
        {
            for(ii = 0; ii < msgs && msginfo[i]; ii++)
            {
                ...
                if(msginfo[ii])
                {
                    ...
                    if(msginfo[ii]->ReplyTo == msginfo[i]->ReplyTo && msginfo[ii]->ReplyTo != 0 &&
                       msginfo[i]->ReplyTo != 0 && ii > i && msginfo[i]->ReplyNext == 0)
                    {
        ...
        for(i = 0; i < msgs; i++)
        {
            for(ii = 0; ii < msgs && msginfo[i]; ii++)
            {
                ...
                if(msginfo[ii])
                {
                    ...
                    if(ii > i && msginfo[ii]->ReplyTo != 0 && msginfo[i]->ReplyNext == 0 &&
...
void linkArea(s_area * area)
{
    ...
    else
    {
        if((area->msgbType & MSGTYPE_JAM) == MSGTYPE_JAM)
        {
            ...
            if(jam_by_crc)
            {
                JamLinkAreaByCRC(area->fileName);
            }
            else
            {
