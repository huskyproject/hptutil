...
unsigned int SortSquishArea(SQSORTptr sqsort,
                            int SqdHandle,
                            int SqiHandle,
                            SQBASEptr sqbase,
                            long idxPos,
...
unsigned int SquishSortArea(s_area * area)
{
    ...
    if(SqlHandle != -1)
    {
        if(lseek(SqlHandle, 0L, SEEK_END) == -1 || (position = tell(SqlHandle)) == -1)
        {
            exit(1);
        }
...
unsigned int SortJamArea(JAMSORTptr jamsort,
                         int IdxHandle,
                         int HdrHandle,
                         long idxPos,
                         long firstnum)
...
unsigned int JamSortArea(s_area * area)
{
    ...
    if(LrdHandle != -1)
    {
        if(lseek(LrdHandle, 0L, SEEK_END) == -1 || (position = tell(LrdHandle)) == -1)
        {
            exit(1);
        }
    ...
    if(lock(HdrHandle, 0, 1) == 0)
    {
        ...
        for(firstnum = 0; i < msgs; i++)
        {
            ...
            if(lseek(HdrHandle, SortIdx.HdrOffset, SEEK_SET) == -1)
            {
                continue;
            }
...
void sortArea(s_area * area)
{
    ...
    if(make)
    {
        if(SortMsgs)
        {
            OutScreen("%d msgs sorted\n", SortMsgs);
        }
        else
        {
            OutScreen("Nothing to sort\n");
        }
    }
    else
    {
